// This algorithm is identical to minimum.js with the exception of:
// * Using -Infinity to initialise the table - line 10
// * Using -Infinity for the i = 0 row - line 16 & 19
// * Using Math.max instead of Math.min - line 24

const calculateMaximumChange = (transaction, cashProvided, coinTray) => {
  ...
  for (let i = 0; i < maxCoinIndex; i++) {
    for (let j = 0; j <= change; j++) {
      if (j === 0) {
        ...
        i === 0
          ? (a[i][j] = -Infinity)
          : a[i][j] = Math.max(a[i-1][j], limits[i] + a[i - 1][j - coins[i] * limits[i]]);
      } else {
        i === 0
          ? (a[i][j] = 1 + a[i][j - coins[i]])
          : (a[i][j] = Math.max(1 + a[i][j - coins[i]], a[i - 1][j]));
      }
    }
  }
...
module.exports = { calculateMaximumChange };
